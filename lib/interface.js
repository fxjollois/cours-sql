...
(function () {
    ...
    function lecture(fichier) {
        ...
        xhr.onload = function (e) {
            ...
            contenu = db.exec("SELECT * FROM sqlite_master WHERE type='table'");
    ...
    function requetage() {
        var bds = $("<div>").attr("id", "choixBD").css("text-align", "center");
        initialisation();
        
        $.ajax({
            url: "bases/",
            success: function (liste) {
                $(liste).find("a:contains(.sqlite)").each(function (i, e) {
                    var bouton = $("<button>").html(e.innerHTML).click(function () {
                        lecture("bases/" + e.innerHTML);
                    });
                    bds.append(bouton);
                });
            }
